...
<div class="max-w-lg mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <div class="text-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-brand mt-2">Match Details</h1>
        ...
        <!-- Match Card -->
        <div class="border-2 {{ $match->isCompleted() ? 'border-gray-300' : 'border-brand' }} rounded-lg overflow-hidden mb-6">
            <!-- Player 1 -->
            <div class="flex items-center justify-between p-4 {{ $match->winner_id === $match->player1_id ? 'bg-green-50' : 'bg-white' }}">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        @if($match->player1)
                        <a href="{{ route('players.show', $match->player1) }}" class="font-semibold hover:text-brand {{ $match->winner_id === $match->player1_id ? 'text-success' : '' }}">
                ...
                <span class="text-3xl font-bold {{ $match->winner_id === $match->player1_id ? 'text-success' : 'text-text-secondary' }}">
                    {{ $match->player1_score ?? '-' }}
                </span>
            </div>

            <div class="text-center py-2 bg-surface-alt text-text-muted font-bold">VS</div>
            ...
            <div class="flex items-center justify-between p-4 {{ $match->winner_id === $match->player2_id ? 'bg-green-50' : 'bg-white' }}">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        @if($match->player2)
                        <a href="{{ route('players.show', $match->player2) }}" class="font-semibold hover:text-brand {{ $match->winner_id === $match->player2_id ? 'text-success' : '' }}">
                ...
                <span class="text-3xl font-bold {{ $match->winner_id === $match->player2_id ? 'text-success' : 'text-text-secondary' }}">
                    {{ $match->player2_score ?? '-' }}
                </span>
            </div>
        </div>
