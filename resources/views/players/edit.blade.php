...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('players.show', $player) }}" class="text-brand hover:underline">&larr; Back to Player Profile</a>
    </div>

    <div class="bg-white rounded-lg shadow-md p-8">
        <h1 class="text-2xl font-bold text-brand mb-6">Edit Player</h1>
        ...
        <form action="{{ route('players.update', $player) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" 
                          id="notes" 
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand focus:border-transparent @error('notes') border-danger @enderror">{{ old('notes', $player->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-danger">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="mt-8 bg-red-50 rounded-lg border border-red-200 p-6">
        ...
        <p class="text-danger text-sm mb-4">Deleting a player cannot be undone. Make sure the player is not registered in any active tournaments.</p>
