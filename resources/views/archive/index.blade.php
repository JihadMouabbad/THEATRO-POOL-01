...
<div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        ...
        <p class="text-gray-500 mt-2">Browse through our completed tournaments and their champions</p>
    </div>
    <div class="flex gap-3">
        <a href="{{ route('tournaments.index') }}" class="px-5 py-2.5 bg-gradient-to-r from-pool-green to-pool-felt text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300">
            🏆 Current Tournaments
        </a>
    </div>
</div>
...
<div class="grid grid-cols-2 gap-4 mb-8">
    <div class="bg-white rounded-xl shadow-lg p-6 text-center card-hover">
        <div class="text-4xl font-black text-pool-green">{{ $stats['total_tournaments'] }}</div>
        <div class="text-sm text-gray-500 font-medium mt-1">Completed Tournaments</div>
    </div>
    <div class="bg-white rounded-xl shadow-lg p-6 text-center card-hover">
        <div class="text-4xl font-black text-pool-green">{{ $stats['total_players_participated'] }}</div>
        <div class="text-sm text-gray-500 font-medium mt-1">Players Participated</div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
    <form action="{{ route('archive.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <select name="year" class="px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-pool-green focus:border-pool-green transition bg-white">
       ...
       class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover group">
        ...
        <div class="p-6">
            <div class="flex items-start justify-between mb-3">
                ...
                <span class="px-3 py-1 bg-gray-100 text-gray-700 text-xs font-bold rounded-full">FINISHED</span>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
                <div class="flex items-center gap-3 p-3 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl">
                    ...
                    <div>
                        ...
                        <div class="font-bold text-gray-800">{{ $tournament->champion->display_name }}</div>
                    </div>
                </div>
            </div>
            @endif
...
<div class="bg-white rounded-2xl shadow-lg p-16 text-center">
    ...
    <h2 class="text-2xl font-bold text-gray-700 mb-2">No Archived Tournaments</h2>
    ...
    <a href="{{ route('archive.index') }}" class="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition">
        Clear Filters
    </a>
    @endif
</div>
