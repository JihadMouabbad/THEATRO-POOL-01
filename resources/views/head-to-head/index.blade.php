...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-text-primary flex items-center gap-3">
        <span class="w-12 h-12 bg-gradient-to-br from-brand to-brand-light rounded-xl flex items-center justify-center text-white text-2xl">⚔️</span>
        Head-to-Head Comparison
    </h1>
    <p class="text-text-secondary mt-2">Compare two players' performance against each other</p>
</div>
...
<div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
    <form action="{{ route('head-to-head.compare') }}" method="GET" class="grid grid-cols-1 md:grid-cols-7 gap-4 items-end">
        <div class="md:col-span-3">
            <label for="player1_id" class="block text-sm font-medium text-text-secondary mb-2">Player 1</label>
            <select name="player1_id" id="player1_id" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-brand focus:border-transparent transition">
        ...
        <div class="md:col-span-3">
            <label for="player2_id" class="block text-sm font-medium text-text-secondary mb-2">Player 2</label>
        ...
        <div class="mt-4 flex justify-center">
            <button type="submit"
                    class="px-8 py-3 bg-gradient-to-r from-brand to-brand-light text-white font-bold rounded-xl hover:shadow-lg transition transform hover:scale-105">
                Compare Players
            </button>
        </div>
    </form>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="h-2 bg-gradient-to-r from-brand to-blue-600"></div>
        <div class="p-6 text-center">
            <div class="w-20 h-20 bg-gradient-to-br from-brand to-blue-600 rounded-full flex items-center justify-center text-white text-3xl font-bold mx-auto mb-4">
                {{ substr($player1->name, 0, 1) }}
            </div>
            <h3 class="text-xl font-bold text-text-primary">{{ $player1->display_name }}</h3>
            <p class="text-text-muted">{{ $player1->name }}</p>
            <div class="mt-4 space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Overall Record</span>
                    <span class="font-bold">{{ $player1->wins }}W - {{ $player1->losses }}L</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Win Rate</span>
                    <span class="font-bold text-brand">{{ $player1->win_rate }}%</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Total Matches</span>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-text-primary mb-4">Head-to-Head Record</h3>
            
            @if($stats['total_matches'] > 0)
                <div class="grid grid-cols-3 gap-4 mb-6">
                    <div class="text-center">
                        <div class="text-3xl font-black text-blue-600">{{ $stats['player1_wins'] }}</div>
                        <div class="text-xs text-text-muted">{{ $player1->display_name }}</div>
                    </div>
                    <div class="text-center">
                        <div class="text-xl font-bold text-gray-400">-</div>
                    </div>
                    <div class="text-center">
                        <div class="text-3xl font-black text-danger">{{ $stats['player2_wins'] }}</div>
                        <div class="text-xs text-text-muted">{{ $player2->display_name }}</div>
                    </div>
                </div>
            ...
            @else
                <div class="text-text-muted py-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="p-6 text-center">
            ...
            <h3 class="text-xl font-bold text-text-primary">{{ $player2->display_name }}</h3>
            <p class="text-text-muted">{{ $player2->name }}</p>
            <div class="mt-4 space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Overall Record</span>
                ...
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Win Rate</span>
                    <span class="font-bold text-brand">{{ $player2->win_rate }}%</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-text-muted">Total Matches</span>
        ...
        <div class="p-4 hover:bg-surface transition">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="text-center">
                        <div class="font-bold {{ $match->winner_id === $player1->id ? 'text-success' : 'text-text-secondary' }}">
                    ...
                    <div class="text-center">
                        <div class="font-bold {{ $match->winner_id === $player2->id ? 'text-success' : 'text-text-secondary' }}">
                ...
                <div class="text-right">
                    <div class="font-medium text-text-primary">{{ $match->tournament->name }}</div>
                    <div class="text-sm text-text-muted">{{ $match->tournament->getRoundName($match->round) }}</div>
                </div>
            </div>
        </div>
        @endforeach
...
<div class="bg-white rounded-2xl shadow-lg p-12 text-center">
    <div class="text-6xl mb-4">⚔️</div>
    <h3 class="text-xl font-bold text-text-primary mb-2">Select Two Players to Compare</h3>
    <p class="text-text-muted">Choose two players from the dropdowns above to see their head-to-head statistics and match history.</p>
</div>
@endif
